...
curl -o Download_Temp/Freeses.html -A "Mozilla/5.0 (X11; Ubuntu; « Linux x86_64; rv:86.0) Gecko/20100101 Firefox/86.0" "https://www.ndr.de/ndr2/wir_sind_die_freeses/podcast4250.xml"

# Sucht im Quelltext nach der URL
URLmp3=$(grep -E '<enclosure url=' Download_Temp/Freeses.html | cut -d= -f2 | cut -c 2- | rev | cut -c 7- | rev)
# cut -d= -f2               Sucht im Text nach dem gleich Zeichen. In diesem Beispiel wird der Text gesucht, der hinter dem ersten gleich Zeichen steht
# cut -c 2-                 Schneidet die ersten zwei Zeichen ab
# rev | cut -c 7- | rev     Schneidet die letzten sieben Zeichen ab
...
sed -i -e 's/Wir sind die Freeses: //g' Download_Temp/NAMEmp3.txt
...
# Manuelle Schleife für Debug Zwecke (in diesem Beispiel würde die Schleife vier mal ausgeführt werden)
#for ((i=1;$i<=4;i++))
